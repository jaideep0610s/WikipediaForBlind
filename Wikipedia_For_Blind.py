...
def stpp(event=0):
    label["text"] = "Press to stop using the application"
    b['state']= tk.NORMAL
    bb['state']= "disabled"
    comm ="Thanks for using our application"
    engine.setProperty('voice', voices[0].id)
...
def callback(flag):
    ...
    if flag is True:
        ...
        loop = rr.after(100, callback, True)
    else:
        # cancel the after method if the flag is False
        stpp()
        rr.after_cancel(loop)

wb = openpyxl.load_workbook('a_tags_book.xlsx')
r = sr.Recognizer()
S = requests.Session()
engine = pyttsx3.init() #tts engine
voices = engine.getProperty('voices')
file_path = os.path.dirname(__file__)
Label(rr, text=" Listening : "
     ).grid(row=0, sticky=W, rowspan=5)
img = ImageTk.PhotoImage(file= file_path + "wikiforblind.jpg")
label = Label(
    rr,
    image=img
)
...
label =Label(rr, text="WIKIPEDIA FOR THE BLIND", bg="dark green", fg="white", font = 'Castellar 30 bold')
label.place(relx=0.5, rely=0.1, anchor=CENTER)

b = Button(rr, text="Start", font = 'Castellar 20 bold', command=lambda: callback(True))
b.place(relx=0.5, rely=0.25, anchor=CENTER)


bb = Button(rr, text="Stop",font = 'Castellar 20 bold', command=lambda: callback(False))
bb.place(relx=0.5, rely=0.5, anchor=CENTER)

label =Label(rr, text="Press Start to run the application", bg="yellow", fg="black",font = 'Castellar 16 bold')
label.place(relx=0.5, rely=0.35, anchor=CENTER)
label= Label(rr, text=" To start press a ", bg="yellow", fg="black",font = 'Castellar 16 bold')
label.place(relx=0.5, rely=0.8, anchor=CENTER)
label= Label(rr, text=" To stop press z ", bg="yellow", fg="black", font = 'Castellar 16 bold')
label.place(relx=0.5, rely=0.9, anchor=CENTER)

label =Label(rr, text=" Press to stop using the application",bg="yellow", fg="black", font = 'Castellar 16 bold',)
label.place(relx=0.5, rely=0.6, anchor=CENTER)

rr.title('Wikipedia For Blind')
