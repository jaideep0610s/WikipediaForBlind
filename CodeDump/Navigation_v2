from bs4 import NavigableString, Tag, BeautifulSoup as soup
...
def get_data(heading):
    print(heading)
    paragraph = ""
    tag = page_soup.find("span", { "id" : heading })
    print(tag)
    print(tag.parent)
    print(tag.parent.next_element)
    tag = tag.parent.next_element
